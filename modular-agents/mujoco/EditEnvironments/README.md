...
These editor scripts were made to adjust the existing CMU humanoid ragdoll (or humanoid ragdolls similar to it) to match in dimensions a character. For this purpose, we use the [Unity Mecanim](https://docs.unity3d.com/Manual/AnimationOverview.html) avatars as a bridge between the two.

The goal is that animations (originating from mocap or keyframed) applied on a skeleton matches the humanoid in the physics simulation without needing retargeting.

For this end, these scripts rely on the [Unity FBX exporter](https://docs.unity3d.com/Packages/com.unity.formats.fbx@2.0/manual/index.html) package. With it you can export MuJoCo humanoids in an fbx, which can then be re-imported as a Mecanim compatible avatar.

This simplifies considerably the mapping between a traditional animated skeleton (what drives a rigged character) and a humanoid in a mujoco simulation. 
...
- `MjScalingEditorWindow.cs` defines the user interface for opening this tool, taking in user parameters, constructing the `ScalingSegment`s, finding the corresponding length information from the reference avatar and calling the scaling operations. As an alternative to MjScalingEditorWindow you can use the  component `ScaligInspector.cs`, it contains the same funcionality.

### Assumptions and Requirements for scaling
